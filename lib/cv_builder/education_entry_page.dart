...
/// Full-screen add/edit form for one [Education] entry, pushed from
/// `EducationStep`. Calls [onSave] with the completed entry when the
...
/// reached via `Navigator.push`, so this constructor runs exactly
/// once for this route's lifetime).
class EducationEntryPage extends StatelessWidget {
  EducationEntryPage({super.key, Education? initial, required this.onSave})
    : isNew = initial == null,
      draft = ValueNotifier(initial ?? Education.blank()),
      attemptedSave = ValueNotifier(false);

  final bool isNew;
  final ValueNotifier<Education> draft;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          isNew ? l10n.cvAddEducationTitle : l10n.cvEditEducationTitle,
        ),
        actions: [
                      ...
                      child: AnimatedBuilder(
                        ...
                        builder: (context, _) {
                          ...
                          return Column(
                            ...
                            children: [
                              FormFieldLabel(label: l10n.cvInstitution),
                              ...
                              TextFormField(
                                ...
                                onChanged: (v) => draft.value = draft.value
                                    .copyWith(institution: v),
                              ),
                              SizedBox(height: gap),
                              FormFieldLabel(label: l10n.cvDegree),
                              SizedBox(height: padding * 0.3),
                              TextFormField(
                              ...
                              SizedBox(height: gap),
                              FormFieldLabel(
                                label: l10n.cvFieldOfStudy,
                                isOptional: true,
                              ),
                              SizedBox(height: padding * 0.3),
                              TextFormField(
                                initialValue: value.fieldOfStudy,
                                textCapitalization: TextCapitalization.words,
                                onChanged: (v) => draft.value = draft.value
                                    .copyWith(fieldOfStudy: v),
                              ),
                              SizedBox(height: gap),
